(function () {
  ...
  class Scroller {
    ...
    init(el) {
      ...
      scrollbarNode.addEventListener(
        "mousedown",
        this.onScrollbarPointerDown.bind(this)
      );
      scrollbarNode.addEventListener(
        "touchstart",
        this.onScrollbarPointerDown.bind(this)
      ...
      document.addEventListener(
        "mousemove",
        this.onScrollbarPointerMove.bind(this)
      );
      document.addEventListener(
        "touchmove",
        this.onScrollbarPointerMove.bind(this)
      ...
      document.addEventListener(
        "mouseup",
        this.onScrollbarPointerUp.bind(this)
      );
      document.addEventListener(
        "touchend",
        this.onScrollbarPointerUp.bind(this)
    ...
    setSize() {
      ...
      const scrollbarFactor =
        scrollwrapWidth !== 0 && sumWidth !== 0
    ...
    onPointerMove(e) {
      ...
      if (result < limitLeft) {
        ...
        scrollbarWidth -= Math.round(
          0.8 * (result - limitRight) * scrollbarFactor
        );
    ...
    onPointerUp(e) {
      ...
      // otherwise
        ...
        const timeToEndpoint = Math.round(
          Math.abs(distanceDelta) / timeFromLastMove
        );
    ...
    onFocus(e) {
      ...
      let endpoint = Math.min(
        Math.max(targetNode.offsetLeft, limitLeft),
        limitRight
      );
    ...
    onScroll(e) {
      ...
      const result = Math.min(
        Math.max(this.get("scrolled") + deltaX, limitLeft),
        limitRight
      );
    ...
    onAnchorClick(e) {
      ...
      const prefix = this.config.prefix;
      const rootNode = this.state.el;
      const targetNode = getElement(
        '[data-anchororiginid="' + anchorid + '"]',
        rootNode
      );
    ...
    onScrollbarPointerMove(e) {
      ...
      const result = Math.min(
        Math.max(delta / scrollbarFactor, limitLeft),
        limitRight
      );
      const scrollbarResult = result * scrollbarFactor;
    ...
    animate(start, stop = 0, speed = 10, animateWidth = false) {
      ...
      const time = Math.max(0.05, Math.min(Math.abs(delta) / speed, 1));
      const scbFactor = this.get("scrollbarFactor");
      ...
      const tick = () => {
        ...
        scbEndpoint =
          currentTime < 1
            ? start * scbFactor +
              delta * this.config.easing(currentTime / time) * scbFactor
            : stop * scbFactor;
